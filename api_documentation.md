...
Read endpoints for the admin dashboard. They bypass `is_public` / ownership checks and include private and unpublished content.

| Method | Path | Params | Defaults |
|---|---|---|---|
| GET | `/admin/stats` | — | — |
| GET | `/admin/users` | query `q`, `status` (`active` \| `banned`), `role` (`admin`), `limit`, `offset` | `limit=20` (max 100), `offset=0` |
| GET | `/admin/users/:id` | path `id` | — |
| GET | `/admin/packs` | query `q`, `type` (`regular` \| `animated`), `user_id`, `visibility` (`public` \| `private`), `limit`, `offset` | `limit=20` (max 100), `offset=0` |
| GET | `/admin/packs/:id` | path `id` | — |

- `/admin/stats` → counters (`totalUsers`, `newUsers30d`, `bannedUsers`, `totalPacks`, `publicPacks`, `totalStickers`, `totalTags`, `pendingPackReports`, `pendingStickerReports`, `totalLikes`, `totalViews`, `totalDownloads`, `totalComments`) plus 12-month series `signupsByMonth`, `packsByMonth`, `downloadsByMonth` (`[{ month: "YYYY-MM", count }]`).
- `/admin/users` → rows with `packCount`, `stickerCount`, `followerCount`; response includes `total`. `q` matches username, email, or full name.
- `/admin/users/:id` → full profile plus `followingCount` and the latest `banReason` (or `null`).
- `/admin/packs` → pack cards with owner info, engagement counts, `pendingReportCount`, `trayIconUrl`; response includes `total`. Use `user_id` to list one user's packs (including private).
- `/admin/packs/:id` → the pack card plus `stickers` (each with like/view/download counts and `pendingReportCount`) and `tags`.

```bash
curl "{{stickerly}}/admin/users?q={{searchText}}&status=active&limit=20&offset=0" \
  -H "Authorization: Bearer {{adminAccessToken}}"

curl "{{stickerly}}/admin/packs?user_id={{userId}}&visibility=private" \
  -H "Authorization: Bearer {{adminAccessToken}}"
```

### Content removal

Soft deletes (`isDeleted = 1`), same semantics as the owner-facing deletes. Removing a pack sends its owner a `pack_removed` notification.

| Method | Path | Params |
|---|---|---|
| DELETE | `/admin/packs/:id` | path `id` — `404` if not found or already deleted |
| DELETE | `/admin/stickers/:id` | path `id` — `404` if not found or already deleted |

```bash
curl -X DELETE {{stickerly}}/admin/packs/{{packId}} \
  -H "Authorization: Bearer {{adminAccessToken}}"

curl -X DELETE {{stickerly}}/admin/stickers/{{stickerId}} \
  -H "Authorization: Bearer {{adminAccessToken}}"
```
...
Report rows are joined with context: `reporter_id` / `reporter_username` / `reporter_profileImg`, the target (`pack_title`, `pack_type`, `trayIconUrl` for pack reports; `sticker_imageUrl`, `pack_id`, `pack_title` for sticker reports), and the content owner (`owner_id`, `owner_username`). Setting a pack report to `actioned` notifies the pack owner (`pack_removed`).
