...
| | |
|---|---|
| **Base URL** | `https://chat.yourdomain.com` (prod) · `http://10.0.2.2:3000` (emulator → your laptop) |
| **Auth** | `x-api-key: <key>` header on every `/api/*` request |
| **Content type** | `application/json` |
| **Transport** | HTTPS in production. No exceptions — the key travels in a header. |

> `10.0.2.2` is how the Android emulator reaches your development machine's `localhost`. On a physical device use your machine's LAN IP.
...
Android blocks cleartext traffic by default. To hit your laptop during development, allow it **for that host only** — never with a blanket `usesCleartextTraffic="true"`, which would also permit plaintext in production.
